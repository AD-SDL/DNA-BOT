...
def run(protocol):
    ...
    def heat_shock():
        ...
        protocol.pause()
        resume = input('Conduct heat shock for by placing competent cells on tempdeck in slot 4 and type yes to resume run.')
        if resume == "yes":
            print("Resuming protocol")
            protocol.resume()
        protocol.delay(seconds=15)
        protocol.pause()
        resume = input('Return Competent cells from tempdeck on slot 4 to tempdeck on slot 10 and type yes to resume run.')
        if resume == "yes":
            print("Resuming protocol")
            protocol.resume()
    ...
    def spot_transformations(
        ...
        def spot(
            ...
            # moving the pipette to the specified location(but why two times?)
            ...
            while max(spot_vols) > 0:
                for index, spot_vol in enumerate(spot_vols):
                    if spot_vol == 0:
                        ...
                        spot(source=transformation_plate.wells(source_wells[index]),
                             target=agar_plate.wells(target_wells[index]), spot_vol=vol)
        ...
        for spotting_tuple in spotting_tuples:
            ...
            for col in unique_cols:
                ...
                p300_pipette.mix(TRANSFORMATION_MIX_SETTINGS[0], TRANSFORMATION_MIX_SETTINGS[1],
                                 transformation_plate.columns_by_name()[col][0])
                # old code:
