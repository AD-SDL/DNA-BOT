...
def run(protocol):
    ...
    def heat_shock():
        ...
        protocol.pause()
        resume = input('Conduct heat shock for by placing competent cells on TC off deck and types yes to resume run.')
        if resume == "yes":
            print("Resuming protocol")
            protocol.resume()
        protocol.delay(seconds=15)
        protocol.pause()
        resume = input('Return Competent cells from TC off deck to tempdeck on slot 4 and types yes to resume run.')
        if resume == "yes":
            print("Resuming protocol")
            protocol.resume()
